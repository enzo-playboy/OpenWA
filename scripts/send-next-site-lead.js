...
async function sendNextSiteLead() {
  ...
  console.log('🔍 Buscando o próximo lead pendente ativo no WhatsApp...');
  ...
  for (let i = 0; i < leads.length; i++) {
    const lead = leads[i];
    await sleep(200); // 200ms de pausa para evitar flood de requisições na API local
    ...
    try {
      ...
      if (!checkRes.ok) {
        console.log(`⚠️ HTTP ${checkRes.status} ao verificar ${lead.name} (${lead.phone}).`);
        continue;
      }
      ...
      if (result.exists && result.whatsappId) {
        ...
        console.log(`⚠️ Telefone ${lead.phone} (${lead.name}) sem WhatsApp ativo. Verificando próximo...`);
      }
    } catch (e) {
      console.log(`Erro ao verificar ${lead.phone} (${lead.name}):`, e.message);
    }
  }
